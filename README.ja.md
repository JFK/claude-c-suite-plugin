...
このリポジトリには Codex 用のマニフェストとスキルアダプタも含まれています：
`.codex-plugin/plugin.json` と `skills/` です。

Codex にインストールしたら、任意の git リポジトリで新しいスレッドを開始し、対応するスキルを呼び出します：
...
```
Claude C-Suite CEO としてこのリポジトリをレビューして。
...
| `/config` | ユーティリティ | `~/.claude/claude-c-suite.json` の管理 — 表示・初期化・設定確認 |
| `/audit` | ユーティリティ | 整合性監査とリリースゲート |
...
Codex 対応は `.codex-plugin/plugin.json` と `skills/` のアダプタで提供しています。このリポジトリをローカルまたは marketplace の Codex プラグインソースとしてインストールし、スキルを読み込ませるために新しい Codex スレッドを開始してください。
...
`~/.claude/claude-c-suite.json` で全 C-Suite コマンドのデフォルト出力言語を設定できます：

```bash
/claude-c-suite:config init   # テンプレートを作成
```
...
| キー | 値 | デフォルト |
|------|-----|-----------|
| `language` | ISO 639-1 コード（`en`, `ja`, `zh`, `ko`, `es`, `fr`, `de` 等） | 質問の言語を自動検出 |
| `version` | プラグインバージョンスタンプ（参考情報） | `init` 時に設定 |

- ファイルが存在しない、または不正な JSON の場合、コマンドは静かに
  フォールバックして質問の言語を自動検出します。
- Codex ではスキルアダプタが `~/.codex/claude-c-suite.json` を先に確認し、なければ上記の Claude Code 用パスにフォールバックします。
- 文章、説明、推奨事項は翻訳されますが、コードブロック、ファイル
  パス、CLI コマンド、構造化ヘッダーは英語のまま維持されます。
- `/claude-c-suite:config` で現在の有効な設定を確認できます。
