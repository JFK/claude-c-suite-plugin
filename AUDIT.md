...
- **Role commands** (13): the executive perspectives (`/ceo`, `/cto`,
  ...
  set of conformance checks, including the `## Language` section that
  honors the user-level `~/.claude/claude-c-suite.json` config.
- **Utility commands** (2): maintainer / configuration tools
  (`/audit`, `/config`). These are exempt from role-specific
  conventions (Question/Review modes, Top 3 cross-references, PhD
